#!/usr/bin/env node
// Quick test: Kontext+LoRA at scale 4.0 (max) on the same 2 cards
  ...
  for (const card of CARDS) {
    ...
    try {
      ...
      const result = await callKontextLora({
        image_url: dataUri, prompt,
        loras: [{ path: LORA_URL, scale: 4.0 }],
      ...
      const outFile = card.baseFile.replace('.png', '-kl4.png');
