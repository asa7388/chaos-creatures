#!/usr/bin/env node
// evolve-all-pro.mjs — Evolve all cards using FLUX Kontext Pro (no LoRA)
// Kontext Pro preserves the existing palette knife style from the base image
// without a LoRA fighting the colors.
//
// Usage: node scripts/evolve-all-pro.mjs --manifest scripts/preview/BASE-iron-r1-manifest.json
...
async function callKontextPro(body) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    if (response.status === 422) throw new Error(`Kontext Pro 422: ${errText}`);
    ...
    throw new Error(`Kontext Pro HTTP ${response.status}: ${errText}`);
  }
}
...
async function evolveCard(card) {
  ...
  const evoFileName = card.fileName.replace('.png', '-evo-pro.png');
...
async function main() {
  ...
  console.log(`\n=== Evolving ${manifest.length} cards (Kontext Pro) ===\n`);
  ...
  const evoManifestName = MANIFEST_PATH.replace(/-manifest\.json$/, '-evo-pro-manifest.json')
    .replace(/^.*\//, '');
