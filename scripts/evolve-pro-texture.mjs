#!/usr/bin/env node
// evolve-pro-texture.mjs — Kontext Pro with aggressive texture preservation
// Full 2-pass chain: BASE → Uncommon → Rare
...
// Much more aggressive style preservation
const STYLE_SUFFIX =
  'CRITICAL: The output MUST look like a thick palette knife oil painting. ' +
  'Apply heavy impasto texture with visible three-dimensional brushstrokes throughout the entire image. ' +
  'Every surface must show thick ridges of paint applied with a palette knife. ' +
  'Keep the exact same color palette, lighting, and background as the original. ' +
  'Do not smooth, soften, or clean up the painterly texture. Do not change the composition or camera angle. ' +
  'The painting style must be rough, textured, and tactile — not smooth or digital.';
...
async function main() {
  ...
  for (const card of CARDS) {
    ...
    try {
      ...
      const uncFile = card.baseFile.replace('.png', '-evo-pt.png');
      ...
      const rareFile = card.baseFile.replace('.png', '-rare-pt.png');
  ...
  writeFileSync(join(PREVIEW_DIR, 'pro-texture-manifest.json'), JSON.stringify(results, null, 2));
