#!/usr/bin/env node
// evolve-test-oil.mjs — Test Owen777 Oil_Painting Kontext LoRA for evolution
// This LoRA was trained specifically for Kontext (not SDXL like EldritchPaletteKnife).
// Should produce more consistent results with the Kontext architecture.
...
async function main() {
  console.log('\n=== Owen777 Oil_Painting LoRA Test ===\n');
  ...
  for (const card of CARDS) {
    ...
    for (const scale of SCALES) {
      console.log(`--- ${card.label} (Oil_Painting @ ${scale}) ---`);
      try {
        const t0 = Date.now();
        const result = await callKontextLora({
          image_url: dataUri,
          prompt,
          loras: [{ path: OIL_LORA_URL, scale }],
          guidance_scale: 2.5,
        ...
        const outFile = card.baseFile.replace('.png', `-oil${tag}.png`);
